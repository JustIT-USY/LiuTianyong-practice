...
class SimProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        data_tr, data_te, class_tr, class_te = train_test_split(data['content'], data['flag'], test_size=0.2,
                                                                random_state=999, stratify=data['flag'])
        train_data = []
        for index,text,label in zip(range(999999),data_tr,class_tr):
            guid = 'train-%d' % index
            text_a = tokenization.convert_to_unicode(str(text))
            label = str(int(label))
    ...
    def get_dev_examples(self, data_dir):
        ...
        data_tr, data_te, class_tr, class_te = train_test_split(data['content'], data['flag'], test_size=0.2,
                                                                random_state=999, stratify=data['flag'])
        test_data = []
        for index,text,label in zip(range(999999),data_te,class_te):
            guid = 'train-%d' % index
            text_a = tokenization.convert_to_unicode(str(text))
            label = str(int(label))
    ...
    def get_test_examples(self, data_dir):
        ...
        for index, test in enumerate(test_df.values):
            guid = 'test-%d' % index
            text_a = tokenization.convert_to_unicode(str(test[2]))
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        logging_hook = tf.train.LoggingTensorHook(tensors=tensors_to_log, every_n_iter=100)  # 修改
