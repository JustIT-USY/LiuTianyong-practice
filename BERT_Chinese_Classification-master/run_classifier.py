...
class SimProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
      ...
      train_data = []
      data_tr, data_te, class_tr, class_te = train_test_split(train_df['text'], train_df['label'], test_size=0.1,
                                                              random_state=9999, stratify=train_df['label'])
      for index, train in zip(range(99999),data_tr,class_tr):
          guid = 'train-%d' % index
          text_a = tokenization.convert_to_unicode(str(train[2]))
          text_b = tokenization.convert_to_unicode(str(train[1]))
          label = str(int(train[3]))
