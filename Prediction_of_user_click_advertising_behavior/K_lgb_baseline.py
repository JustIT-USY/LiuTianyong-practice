...
if __name__ == "__main__":
    ...
    params = {
        ...
        "bagging_seed": 10,
        "lambda_l1": 0.9,  # l1正则
        'lambda_l2': 0.002,  # l2正则
        # "verbosity": -1,
        # "nthread": -1,  # 线程数量，-1表示全部线程，线程越多，运行的速度越快
        'metric': {'binary_logloss', 'auc', 'rmse'},  ##评价函数选择
        "random_state": 80,  # 随机数种子，可以防止每次运行的结果不一致
    }
    # params = {
    #     'subsample_freq': 1,
    #     'subsample': 0.85,
    ...
    folds = StratifiedKFold(n_splits=K, shuffle=True, random_state=98)
    ...
    for fold_, (trn_idx, val_idx) in enumerate(folds.split(train_x, train_y)):
        ...
        y_pred = clf.predict(train_x.iloc[val_idx])
        ...
        fold_importance_df.to_csv('feature/{}.csv'.format(fold_ + 1))
    ...
    res_data.to_csv('再不上分死了/submission_lgb_10K_{}.csv'.format(sum(K_auc) / K), index=False, encoding='utf-8')
