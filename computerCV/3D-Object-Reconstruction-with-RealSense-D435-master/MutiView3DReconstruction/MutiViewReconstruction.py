...
RealSense = rgbdTools.Camera(fx=614.2888, fy=614.3722, cx=321.6398, cy=237.2958)
...
for j in range(cloud_number):
    ...
    for ind, x in enumerate(xl):
        ...
        if abs(TablePlane.a * x + TablePlane.b * y + TablePlane.c * z + TablePlane.d) / (
                TablePlane.a ** 2 + TablePlane.b ** 2 + TablePlane.c ** 2) ** 0.5 < 0.008:
            x, y, z = rgbdTools.getPosition(RealSense, depth, yl[ind], xl[ind])
            Pt3.append([x, y, z])
    ...
    for mm in range(0, 480):
        for nn in range(0, 640):
            ...
            if y < (TablePlane.a * x + TablePlane.c * z + TablePlane.d) / (
                    -TablePlane.b) - 0.01 and z > 0.15 and z < 0.35:
                Pt4.append([x, y, z])
